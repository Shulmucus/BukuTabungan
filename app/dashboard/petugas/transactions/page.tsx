...
export default function PetugasTransactionsPage() {
    ...
    const columns = [
        ...
        {
            key: 'nasabah', header: 'Nasabah', render: (row: any) => (
                <div>
                    <p className="font-medium text-sm">{row.nasabah?.user?.full_name || '-'}</p>
                    <p className="text-xs text-surface-400 font-mono">{row.nasabah?.account_number || ''}</p>
                </div>
            )
        },
        {
    ...
    return (
        <div>
            <Header title="Histori Transaksi" subtitle="Lihat semua transaksi nasabah" />
            ...
            <div className="glass-card p-4 mb-6">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    />
                    <FormInput label="Dari Tanggal" type="date" value={filters.date_from} onChange={(e) => { setFilters((f) => ({ ...f, date_from: e.target.value })); setPage(1); }} />
                    <FormInput label="Sampai Tanggal" type="date" value={filters.date_to} onChange={(e) => { setFilters((f) => ({ ...f, date_to: e.target.value })); setPage(1); }} />
                </div>
            </div>
